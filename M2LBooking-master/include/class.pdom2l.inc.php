...
class PdoM2l
{
    ...
    {
        $req = "select departement.code as codeDep, departement.nom as nomDep, count(ligue.id) as nbLigue from departement left join ligue on ligue.codeDep = departement.code group by departement.code";
    ...
    public function getInfosLigue($idLigue){
        $req = "select ligue.id as idLigue, ligue.nom as nomLigue, ligue.adresseRue as adrLigue, ligue.cp as cpLigue, 
                ligue.ville as villeLigue, ligue.emailContact as emailLigue, ligue.urlSiteWeb as urlLigue, ligue.tel
                 as TelLigue, ligue.urlImage as urlImage, discipline.olympique as DiscLigue
		        from  ligue,discipline where ligue.id ='$idLigue' and ligue.idDis = discipline.id";
